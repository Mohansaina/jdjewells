...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { shippingName, shippingEmail, shippingPhone, shippingAddress, shippingCity, shippingZip, items } = body;
    ...
    // Server-side price verification and database operations executed inside a transaction
        ...
        if (itm.customConfig) {
          ...
          if (diamondVdbId) {
            ...
            if (vdbDiamond) {
              ...
              throw new Error(`Custom diamond not found in active registries: ${diamondVdbId}`);
            }
          }

          // Create configuration record inside transaction
          const configRecord = await tx.configuration.create({
            data: {
          ...
          const product = await tx.product.findUnique({ where: { id: itm.productId } });
          if (!product) {
            throw new Error(`Product not found: ${itm.productId}`);
          }
          itemPrice = product.price;
        } else if (itm.diamondId) {
          const diamond = await tx.diamond.findUnique({ where: { id: itm.diamondId } });
          if (diamond) {
            ...
            if (vdbDiamond) {
              ...
              throw new Error(`Diamond not found in active registries: ${itm.diamondId}`);
            }
          }
        } else {
          throw new Error("Each order item must specify a productId, diamondId, or customConfig");
        }
